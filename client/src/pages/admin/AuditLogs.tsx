...
import {
  ...
  Settings, RefreshCcw, Bell, Monitor, ChevronDown,
} from 'lucide-react';
...
type AuditOutcome = 'GRANTED' | 'DENIED_ROLE' | 'DENIED_RELATIONSHIP' | 'DENIED_CONTEXT' | 'DENIED_SOD' | 'ERROR';
type DenyLayer = 'ROLE' | 'RELATIONSHIP' | 'CONTEXT' | 'SOD' | 'ALL' | '—';
...
export function AuditLogsScreen({
  ...
  const [layerFilter, setLayerFilter] = useState<DenyLayer | 'ALL'>('ALL');

  useEffect(() => {
    api.me().then(setMe).catch(console.error);
    api.auditLogs().then(res => setLogs(res.data)).catch(console.error);
  ...
  const filtered = useMemo(() => {
    return logs.filter((log) => {
      const layer = (log.metadata?.layer as string) ?? '—';
  ...
  }, [logs, search, outcomeFilter, layerFilter]);

  const handleCopyJson = (log: any) => {
    navigator.clipboard.writeText(JSON.stringify(log, null, 2)).catch(console.error);
  };
  ...
  return (
    <Shell
      ...
      searchPlaceholder="Search Principal/UUID..."
      ...
      topIcons={
        <>
          ...
          <button className="icon-chip" type="button" aria-label="Refresh" onClick={() => {
            api.auditLogs().then(res => setLogs(res.data)).catch(console.error);
            api.auditStats().then(setStats).catch(console.error);
        ...
        <div className="filter-bar panel-card">
          ...
          <div className="filter-field select">
            ...
            <select
              className="select-box"
              value={layerFilter}
              onChange={(e) => setLayerFilter(e.target.value as DenyLayer | 'ALL')}
            >
          ...
          <button
            ...
            type="button"
            onClick={() => { setSearch(''); setOutcomeFilter('ALL'); setLayerFilter('ALL'); }}
          >
            CLEAR
          </button>
        </div>
          ...
          {filtered.map((log) => (
            ...
            >
              ...
              >
                ...
                <span>
                  <span className={`pill ${log.outcome === 'GRANTED' ? 'ok' : 'deny'}`}>
                    {log.outcome}
                  </span>
                </span>
